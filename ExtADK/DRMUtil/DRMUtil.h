...
{
    kDRMEventIDDocumentBeforeOpen,     // 문서가 열리기 직전
    kDRMEventIDDocumentAfterOpen,      // 문서가 열린 후
    kDRMEventIDDocumentBeforeSave,     // 문서가 저장되기 직전
    kDRMEventIDDocumentAfterSave,      // 문서가 저장 된 후
    kDRMEventIDDocumentBeforeEdit,     // 문서의 Content(e.g, 이미지, 테이블 내용)이 변경되기 전
    kDRMEventIDDocumentPreview,        // 문서 미리보기 내용 표시 전(e.g, 인쇄 시)
    kDRMEventIDAutoSaveBefore,         // 자동저장 파일 생성 전
    kDRMEventIDTempFileCreate,         // 임시 파일 생성 전
    kDRMEventIDTempFileDelete,         // 임시 파일 삭제
    kDRMEventIDInitApp,                // 프로그램 실행 시
    kDRMEventIDBeforeMacroFunction,    // 매크로 기능 수행 전
